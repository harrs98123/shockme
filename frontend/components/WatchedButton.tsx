...
export default function WatchedButton({ movie, variant }: Props) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!user) return;
    api.get(`/watched/ids?media_type=${mediaType}`).then((r) => {
      setIsWatched((r.data as number[]).includes(movie.id));
    }).catch(() => { });
  ...
  const toggle = async () => {
    ...
    try {
      if (isWatched) {
        await api.delete(`/watched/${movie.id}?media_type=${mediaType}`);
        setIsWatched(false);
      } else {
  ...
  return (
    <button
      ...
      style={{
        ...
        transition: 'all 0.2s',
        minWidth: isPurple ? 220 : 0,
        opacity: loading ? 0.6 : 1
      }}
    >
      {isWatched ? '✅' : '👁️'} {isWatched ? 'Watched' : 'Mark as Watched'}
    </button>
  );
}
