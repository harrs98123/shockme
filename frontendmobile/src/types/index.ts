 ...
 * Shared domain types.
 *
 * Ported from `frontend/lib/types.ts` — the FastAPI response shapes are
 * identical for web and mobile, so this file must stay in step with it.
 * Admin-console-only types (AdminStats, SystemHealth, AdminSocialPost and the
 * recent-activity items) are intentionally omitted; see MIGRATION_PLAN.md §7.
 */
...
export interface AuthToken {
  access_token: string;
  /**
   * The web client discards this; mobile persists it and calls /auth/refresh.
   * Nullable because `schemas.Token` declares it `Optional[str] = None` — a
   * response without it means this session cannot be refreshed.
   */
...
/** TMDB `watch/providers` — the web app types this as `Record<string, any>`. */
...
export interface Media {
  ...
  reason?: string;
  'watch/providers'?: {
    /** Keyed by ISO 3166-1 country code, e.g. `US`, `IN`. */
...
export type PersonCredit = PersonRole;
...
// ─── Curated content (public read endpoints under /admin/*) ────────────
