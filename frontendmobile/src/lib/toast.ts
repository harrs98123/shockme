/**
 * Toast state manager, ported from `frontend/lib/toast.tsx`.
 *
 * The manager is a framework-agnostic pub/sub store, so it moves across
 * unchanged — only the renderer is platform-specific. `ToastHost` subscribes to
 * it and draws native views where the web app drew a fixed-position div.
 *
 * Changed for React Native: `icon` is a lucide icon name rather than a
 * ReactNode, and `action.onClick` takes no MouseEvent.
 */
...
export interface ToastOptions {
  ...
  /** Lucide icon name; the host resolves it. Omit to use the type's default. */
  icon?: string;
  action?: ToastAction;
  cancel?: {
    label: string;
